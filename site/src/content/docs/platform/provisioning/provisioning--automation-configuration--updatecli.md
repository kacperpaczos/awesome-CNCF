---
title: Updatecli
description: "Updatecli is a declarative dependency management tool for Git repositories."
domain: "platform"
category: "Provisioning"
subcategory: "Automation & Configuration"
project_id: "provisioning--automation-configuration--updatecli"
maturity: null
openapi_status: todo
---

# Updatecli

![Updatecli](/logos/686597cac9e75c36b3d1127954d0dce08e4e80df30e8773ecb858255733a6b18.svg)

**Domena:** [Platforma i infrastruktura](../index.md) · **Kategoria:** [Provisioning](index.md) · Automation & Configuration  
**Open source:** tak

## Opis

Updatecli is a declarative dependency management tool for Git repositories.

## Use cases

- Updatecli: Updatecli is a declarative dependency management tool for Git repositories
- Automatyzacja provisioningu infrastruktury jako kod
- Standaryzacja konfiguracji środowisk dev/stage/prod
- Compliance i audyt zmian w klastrach Kubernetes
- Budowa i utrzymanie platformy Kubernetes / cloud native

_Sekcja wygenerowana heurystycznie — popraw według własnego doświadczenia._

## Alternatywy

- [KubeEdge](/platform/provisioning/provisioning--automation-configuration--kubeedge/) — Ta sama subkategoria: Automation & Configuration · CNCF graduated · ⭐ 7,475
- [Cloud Custodian](/platform/provisioning/provisioning--automation-configuration--cloud-custodian/) — Ta sama subkategoria: Automation & Configuration · CNCF incubating · ⭐ 6,006
- [OpenYurt](/platform/provisioning/provisioning--automation-configuration--openyurt/) — Ta sama subkategoria: Automation & Configuration · CNCF incubating · ⭐ 1,964
- [metal3-io](/platform/provisioning/provisioning--automation-configuration--metal3-io/) — Ta sama subkategoria: Automation & Configuration · CNCF incubating · ⭐ 745

## Dokumentacja

- [Strona główna](https://www.updatecli.io/)

### Repozytoria

- [updatecli](https://github.com/updatecli/updatecli)

## OpenAPI

> **TODO:** Pełna specyfikacja w submodule `openapi/provisioning--automation-configuration--updatecli` — aktualizacja przez CI/CD.

| | |
|---|---|
| Status | `todo` |
| Docelowy URL | [/openapi/provisioning--automation-configuration--updatecli/](/openapi/provisioning--automation-configuration--updatecli/) |
| Submodule | `openapi/provisioning--automation-configuration--updatecli` |
| Szukaj w repo | [źródło](https://github.com/updatecli/updatecli/tree/HEAD) |

## Metadane CNCF

| Pole | Wartość |
|------|---------|
| Accepted | — |
| Graduated | — |
| Incubating | — |
| Archived | — |
| CLOMonitor | — |

## GitHub

- Gwiazdki GitHub: **930**
- Licencja: **Apache License 2.0**
- Tematy: `continuous-updates`, `dependency-manager`, `devops`, `gitops`, `good-first-issue`, `hacktoberfest`
